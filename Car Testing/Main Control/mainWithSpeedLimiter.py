...
# Same as main.py except corrects steering_dc less as we go faster (time goes on)
...
accRamp = np.linspace(0.018, 0.01, int(SIMULATION_TIME // 0.008))
accRamp = accRamp.tolist()

# Less correction as speed increases 
speedLimiterNP = np.linspace(1.0, SIMULATION_TIME - 0.2, int(SIMULATION_TIME // 0.008))
...
def throttle_thread():
    ...
    while not stop_event.is_set() and (time.time() - throttle_start_time < SIMULATION_TIME):
        ...
        # Only increment throttle if we are not at MAX
        if (throttle_dc <= 19.83):
            try:
        ...
        # Uncomment to brake when top speed is hit
        '''
...
def main():
    ...
    try:
        ...
        # BRAKE
        # If mux thread did not set to 15
        if (throttle_dc > 15.8):
            lgpio.tx_pwm(HANDLE, THROTTLE_PIN, FREQUENCY, 10.1)
            time.sleep(1.0)
        # NEUTRAL
